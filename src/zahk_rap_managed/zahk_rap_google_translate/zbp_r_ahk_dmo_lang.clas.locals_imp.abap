CLASS lhc_zr_ahk_dmo_lang DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING
              REQUEST requested_authorizations FOR Language " Language -> alias of Behavior Def. ZR_AHK_DMO_LANG
              RESULT result.
...
CLASS lhc_zr_ahk_dmo_lang IMPLEMENTATION.
  METHOD get_global_authorizations.
  ENDMETHOD.

  METHOD TranslateText.
    READ ENTITIES OF zr_ahk_dmo_lang IN LOCAL MODE
         ENTITY Language " Language -> alias of Behavior Def. ZR_AHK_DMO_LANG
         ALL FIELDS WITH CORRESPONDING #( keys )
         RESULT DATA(lt_selected).

    LOOP AT lt_selected INTO DATA(ls_selected) WHERE TargetLanguage IS NOT INITIAL.

      DATA(ld_translated) = NEW zcl_ahk_google_integration( )->translate_text(
                                    id_target_language = to_lower( ls_selected-TargetLanguage )
                                    id_text            = CONV #( ls_selected-SourceText ) ).

      MODIFY ENTITIES OF zr_ahk_dmo_lang IN LOCAL MODE
